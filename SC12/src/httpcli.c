...
int httpGet(char *request_line, char *request_buf, int maxlen, char *dns)
{
  ...
  if (help_ptr)
  {
    ...
    *help_ptr = 0; // divide the combined string into authorization and address
    ...
    auth_length = base64encode((unsigned char *) uri_buf,
        (unsigned char *) Base64EncodeBuf, strlen(uri_buf),
        MAX_BASE64_ENCODE_BUFSIZE);
  }
  else
  {
  ...
  if (inet_addr(uri_ptr, &addr.sin_addr.s_addr) != 0)
  {
    ...
    error = gethostbyname(dns, uri_ptr, &ttl, &addr.sin_addr.s_addr,
        ipHttpServerIPStr);
    if (error != 1)
    {
      printf("\r\n\nHTTP-Client Error: %d DNS failed", error);
      return HTTP_DNS_FAILED;
    }
#endif
  }
  ...
  if (sd == API_ERROR)
  {
    printf("\r\n\nHTTP-Client Error: %d socket open failed", error);
    return 0;
  }
  ...
  if (connect(sd, (const struct sockaddr *) &addr, &error) == API_ERROR)
  {
    ...
    {
      printf("\r\n\nHTTP-Client Error: %d socket connect failed", error);
    }
    goto EndTransfer;
  }
  ...
  {
    printf("\r\n\nHTTP-Client Error: %d sending request", error);
    goto EndTransfer;
  }
  ...
  do
  {
    rcvd = recv(sd, ReceiveBuffer, HTTPCLI_RCVBUFSIZE, MSG_TIMEOUT, 10000L,
        &error);
    if (error == 0)
    {
  ...
  *help_ptr = 0; // Terminate the received-page-string
  ...
  EndTransfer: if (closesocket(sd, &error) < 0)
  {
