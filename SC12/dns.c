                    ...
                    char *dest)              // The IP as text (make sure it is long enough)
{
  ...
  buffer[len1++]=0; buffer[len1++]=1; // query type is IP address
  buffer[len1++]=0; buffer[len1++]=1; // query class is also IP address
  ...
  // send data
  if (  API_ERROR==sendto(sd, (char *)buffer, len1, MSG_BLOCKING, (const struct sockaddr * )&addr, &error))
  {
  ...
  len2=recvfrom(sd, (char *)buffer, BUFSIZE, MSG_TIMEOUT, 10000,(struct sockaddr *)&addr,&error);
  closesocket(sd,&error);
  if (len2<=len1)
  {
  ...
  while(nRR--)
  {
     if((j=dn_unpackrr(&myrr, (char *)buffer,(char *)&buffer[i])) == -1)             /* get answer-rr from buffer */
     {
